...
(() => {
  ...
  async function loadAll() {
    ...
    await Promise.all(
      tasks.map(
        async ([table, key, order]) => {
          try {
            state[key] = await query(
              table,
              '*',
              order,
              false
            ...
            /*
             * View boleh kosong / belum tersedia.
             * Tabel utama tetap harus berhasil.
             */
    ...
    try {
      state.journalLines = await query(
        'journal_lines',
        '*',
        'line_no',
        true
  ...
  function accountId(code) {
    return account(code)?.id || null;
  }

  function cashId() {
    return state.cashAccounts[0]?.id || null;
  }

  function ensureAccount(code, label) {
    const id = accountId(code);

    if (!id) {
      throw new Error(
        `Akun ${code} (${label}) belum ada. Jalankan SQL database KARSA terlebih dahulu.`
      );
    }
  ...
  function ensureCash() {
    ...
    if (!id) {
      throw new Error(
        'Belum ada Kas/Bank. Buat minimal satu rekening di Supabase atau jalankan SQL setup KARSA.'
      );
    }
  ...
  async function createJournal({
    ...
    const debit = clean.reduce(
      (s, x) => s + x.debit,
      0
    );

    const credit = clean.reduce(
      (s, x) => s + x.credit,
      0
    );
    ...
    if (
      ...
      throw new Error(
        `Jurnal tidak balance. Debit ${money(
          debit
        )} ≠ Kredit ${money(credit)}.`
      );
    }
    ...
    for (const line of clean) {
      if (!line.account_id) {
        throw new Error(
          'Ada akun jurnal yang belum tersedia. Jalankan SQL setup.'
        );
      }
    }
          ...
          <div class="field full">
            <label>
              ...
              <span class="field-help">
                format: nama|kelompok|qty|harga satuan,
                satu per baris
              </span>
              ...
              <textarea
                name="components"
                rows="8"
                placeholder="Kain Utama|bahan|1|45000
...
Ongkos Produksi|produksi|1|15000"
              ></textarea>
            </label>
          </div>
        ...
        [
          ...
          <div class="attention-item">
            ...
            <div>
              ...
              <small>
                Jurnal tersimpan di database Supabase.
              </small>
            </div>
          </div>
          `,
          ...
          <div class="attention-item">
            ...
            <div>
              ...
              <small>
                HPP dihitung dari komponen produk.
              </small>
            </div>
          </div>
          `,
          ...
          <div class="attention-item">
            ...
            <div>
              ...
              <small>
                Periksa jatuh tempo pada menu Piutang.
              </small>
            </div>
          </div>
          `,
          ...
          <div class="attention-item">
            ...
            <div>
              ...
              <small>
                Periksa kewajiban pada menu Hutang.
              </small>
            </div>
          </div>
          `
        ...
        },
        {
          ...
          render: r => {
            ...
            return Math.abs(
              ...
              ? '<span class="badge ok-badge">BALANCE</span>'
              : '<span class="badge danger-badge">SELISIH</span>';
          }
        }
      ],
      rows
  ...
  function exportData(type) {
    ...
    if (type === 'all') {
      downloadCSV(
        `KARSA-Finance-Semua-${dateToday()}.csv`,
        state.transactions
      );
  ...
  function exportWorkbook() {
    if (!window.XLSX) {
      toast(
        'Library Excel belum termuat. Pastikan koneksi internet tersedia.',
        false
      );
      ...
      [
        ...
        },
        {
          Item:
            'HPP',
          Keterangan:
            'Qty x Harga Satuan; total produk = SUM komponen'
        },
        {
          Item:
            'Laba Bersih',
        ...
        },
        {
          Item:
            'Spreadsheet',
          Keterangan:
            'File XLSX dapat dibuka/import ke Google Sheets.'
        }
      ]
    );
    ...
    lr[1][1] = {
      f: `SUMIF(Jurnal!G:G,"Penjualan",Jurnal!I:I)`
    };

    lr[2][1] = {
      f: `SUMIF(Jurnal!G:G,"HPP",Jurnal!H:H)`
    };
    ...
    lr[4][1] = {
      f: `SUMIF(Jurnal!G:G,"Beban*",Jurnal!H:H)`
    };
    ...
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet(lr),
      'Laba_Rugi'
    );

    XLSX.writeFile(
      wb,
      `KARSA-Finance-${dateToday()}.xlsx`
    );

    toast(
      'Workbook Excel berhasil dibuat. XLSX dapat dibuka di Microsoft Excel atau Google Sheets.'
    );
  }
  ...
  /*
   * ============================================================
   * INIT — FIX LOADING SUPABASE AUTH
   * ============================================================
   *
   * Jangan melakukan query database langsung dari callback
   * onAuthStateChange.
   *
   * Supabase Auth memiliki lock internal. Jika loadAll()
   * dipanggil langsung dari callback Auth, proses bisa
   * menggantung pada beberapa kondisi.
   */
  ...
  async function init() {
    ...
    try {
       ...
       *
       * Penting:
       * loadAll() TIDAK dipanggil langsung di callback.
       *
       * setTimeout(..., 0) membuat callback database berjalan
       * setelah callback Auth selesai.
       */
